...
function DashboardPage() {
  ...
  const [top20Only, setTop20Only] = useState(true);
  ...
  useEffect(() => {
    ...
    fetchJson('/api/rising-amount-rank?limit=80')
      .then((next) => {
        if (!cancelled) setDailyRank(next);
      })
      .catch((error) => {
        if (!cancelled) setDailyRank((current) => ({
          ...current,
          ok: false,
  ...
  const tableRows = useMemo(() => {
    let rows = [...dailyRankRows];
    if (sectorFilter !== 'all') rows = rows.filter((stock) => stock.sector === sectorFilter);
    rows = sortDashboardStocks(rows, 'tradeAmount');
    return rows.slice(0, top20Only ? 20 : 100);
  }, [dailyRankRows, sectorFilter, top20Only]);
  const topListHighlightCodes = useMemo(() => {
    return new Set(dailyRankRows.slice(0, 20).map((stock) => String(stock.code || '')));
  }, [dailyRankRows]);
  ...
  const boardTitle = '상승 거래대금순';
  const amountProviderFallback = 'Kiwoom OpenAPI+ opt10032 rising-filter';
  const sectionDescription = '상승 거래대금순 상위 20개 종목을 카드 안에 음영 표시';
    ...
    <main className="app-shell wide-shell">
      <header className="page-header">
        ...
        <div className="stat-strip">
          ...
          <Metric label="표시 거래대금" value={`${fmtTradeAmount(stats.top20TradeAmount)} (${ratio(stats.top20TradeAmount, stats.totalTradeAmount)})`} />
      ...
      {(!snapshot.ok || snapshot.message) && (
        <section className="notice">
          ...
          <p>{snapshot.message || snapshot.error || '`Start_Money_All.bat` 실행 후 키움 로그인을 완료하세요.'}</p>
        </section>
      )}
      ...
      <div className="dashboard-layout">
        <div className="main-col">
          ...
          <section className="sector-grid" aria-label="섹터별 거래대금 보드">
            {sectors.length ? sectors.map((sector, index) => (
              <SectorCard
                ...
                highlightCodes={topListHighlightCodes}
          ...
          <section className="legend-row">
            <span>{`${boardTitle}: ${providerText(dailyRank.provider || amountProviderFallback)} · ${dailyRank.updatedAt ? timeText(dailyRank.updatedAt) : '조회 대기'}`}</span>
          ...
          <section className="table-toolbar">
            ...
            <div className="table-controls">
              ...
              <span className="chip fixed-chip">등락률 + · 거래대금순</span>
            </div>
          </section>
          ...
          <section className="table-wrap">
            <table>
              ...
              <tbody>
                ...
                )) : <tr><td colSpan="12" className="empty-cell">표시 가능한 종목이 없습니다.</td></tr>}
              </tbody>
            </table>
          </section>
        </div>
...
function SectorCard({ sector, rank, selected, highlightCodes, onSelect }) {
    ...
    <button className={`sector-card ${selected ? 'selected' : ''} ${sector.hotFlowCount ? 'alerting' : ''}`} onClick={onSelect} type="button">
      ...
      <div className="stock-mini-list">
        {stocks.length ? stocks.map((stock, index) => (
          <button key={stock.code} className={`sc-stock-row ${highlightCodes?.has(String(stock.code || '')) ? 'in-top-list' : ''}`} type="button" onClick={(event) => { event.stopPropagation(); go(`/stock/${stock.code}`); }}>
...
function StockRow({ stock, index }) {
  ...
  return (
    <tr className={stock.flowHot ? 'hot-row' : ''}>
      ...
      <td className="num flow-cell"><FlowPair stock={stock} /></td>
...
function FlowPair({ stock }) {
  return (
    <span className="flow-pair">
      <span><b>1분</b>{fmtTradeAmount(stock.flow60sTradeAmountMillion)}</span>
      <span><b>3분</b>{fmtTradeAmount(stock.flow180sTradeAmountMillion)}</span>
    </span>
  );
}
...
function BurstPanel({ alerts, threshold }) {
  const rows = buildBurstRows(alerts).slice(0, 60);
  return (
    <section className="side-panel">
      ...
      <p className="side-sub">1분/3분 거래대금 {fmtTradeAmount(threshold || 1000)} 이상 종목</p>
      <div className="burst-list">
        {rows.length ? rows.map((alert) => (
          <div className="burst-row" key={alert.key || `${alert.code}-${alert.detectedAt}`}>
            <div>
              <strong>{alert.name}</strong>
              <small>{alert.sector} · {alert.code}</small>
            </div>
            <span className="burst-flow-pair">
              <span><b>1분</b>{fmtTradeAmount(alert.flow60sTradeAmountMillion)}</span>
              <span><b>3분</b>{fmtTradeAmount(alert.flow180sTradeAmountMillion)}</span>
            </span>
...
function buildBurstRows(alerts) {
  const grouped = new Map();
  [...alerts]
    .sort((a, b) => new Date(b.detectedAt || 0) - new Date(a.detectedAt || 0))
    .forEach((alert) => {
      const code = String(alert.code || alert.key || '');
      const current = grouped.get(code) || {
        ...alert,
        key: code,
        count: 0,
        flow60sTradeAmountMillion: 0,
        flow180sTradeAmountMillion: 0,
      };
      const flow60 = number(alert.flow60sTradeAmountMillion) || (number(alert.windowSec) === 60 ? number(alert.tradeAmountMillion) : 0);
      const flow180 = number(alert.flow180sTradeAmountMillion) || (number(alert.windowSec) === 180 ? number(alert.tradeAmountMillion) : 0);
      current.flow60sTradeAmountMillion = Math.max(number(current.flow60sTradeAmountMillion), flow60);
      current.flow180sTradeAmountMillion = Math.max(number(current.flow180sTradeAmountMillion), flow180);
      current.tradeAmountMillion = Math.max(
        number(current.tradeAmountMillion),
        number(alert.tradeAmountMillion),
        current.flow60sTradeAmountMillion,
        current.flow180sTradeAmountMillion,
      );
      current.count += number(alert.count) || 1;
      if (new Date(alert.detectedAt || 0) >= new Date(current.detectedAt || 0)) {
        current.detectedAt = alert.detectedAt;
        current.name = alert.name || current.name;
        current.sector = alert.sector || current.sector;
      }
      grouped.set(code, current);
    });
  return [...grouped.values()].sort((a, b) => new Date(b.detectedAt || 0) - new Date(a.detectedAt || 0));
}
...
function providerText(value) {
  ...
  return text
    ...
    .replace('Kiwoom OpenAPI+ opt10032 rising-filter', '키움 상승 거래대금상위 TR')
