...
const EXTERNAL_BRIDGE_ONLY = ['1', 'true', 'yes', 'on'].includes(String(process.env.KIWOOM_EXTERNAL_BRIDGE_ONLY || '1').toLowerCase());
...
const EXCLUDED_PRODUCT_RE = /(ETF|ETN|ELW|스팩|기업인수목적|리츠|KODEX|TIGER|ACE|SOL|RISE|KOSEF|HANARO|KBSTAR|ARIRANG|TIMEFOLIO|히어로즈|PLUS|KOACT|UNICORN|TREX|FOCUS|KTOP)/i;
...
app.get('/api/provider', (req, res) => {
  res.json({
    provider: 'External sector classification + Kiwoom OpenAPI+',
    mode: 'local-bridge',
    ...
    dataBoundary: 'Sectors are classified from an external industry source; price, volume, ranking, and trade amount stay on Kiwoom OpenAPI+',
...
app.get('/api/health', async (req, res) => {
  ...
  res.status(health.ok ? 200 : 503).json({
    server: true,
    dataBoundary: 'External sector classification only; Kiwoom OpenAPI+ for market data',
    bridge: health,
  });
});
...
app.get('/api/rising-amount-rank', async (req, res) => {
  const limit = Math.max(1, Math.min(toInt(req.query.limit, 50), 100));
  const markets = String(req.query.markets || '').trim();
  const params = new URLSearchParams({ limit: String(limit) });
  if (markets) params.set('markets', markets);
  const bridge = await bridgeJson(`/rising-amount-rank?${params.toString()}`, {}, 120000);
  if (bridge.ok) return res.json(normalizeDailyAmountRankPayload({
    ...bridge,
    provider: bridge.provider || 'Kiwoom OpenAPI+ opt10032 rising-filter',
  }));

  const allRank = await bridgeJson(`/daily-amount-rank?${params.toString()}`, {}, 120000);
  if (allRank.ok) {
    const rows = (allRank.items || [])
      .filter((row) => isRisingRankRow(row))
      .sort((a, b) => Number(b.tradeAmountMillion || 0) - Number(a.tradeAmountMillion || 0))
      .slice(0, limit)
      .map((row, index) => ({ ...row, rank: index + 1, sourceLabel: '키움상승거래대금순TR' }));
    return res.json(normalizeDailyAmountRankPayload({
      ...allRank,
      provider: 'Kiwoom OpenAPI+ opt10032 rising-filter',
      criteria: { ...(allRank.criteria || {}), rank: 'rising-daily-trade-amount', changeFilter: 'change-rate-positive', sort: 'tradeAmountMillion-desc' },
      items: rows,
      message: bridge.error || bridge.message || null,
    }));
  }

  res.status(503).json(normalizeDailyAmountRankPayload({
    ok: false,
    provider: 'Kiwoom OpenAPI+ opt10032 rising-filter',
    items: [],
    message: bridge.error || bridge.message || '키움 상승 거래대금순을 불러오지 못했습니다.',
  }));
});
...
app.listen(PORT, () => {
  console.log(`[money-dashboard] server listening on http://127.0.0.1:${PORT}/`);
  console.log(`[money-dashboard] local alias http://localhost:${PORT}/`);
  console.log(`[money-dashboard] bridge ${BRIDGE_URL}`);
  ensureBridgeStarted().catch((error) => {
    console.warn(`[money-dashboard] bridge auto-start skipped: ${error?.message || error}`);
  });
});
...
function enrichFlow(snapshot, sortKey) {
  ...
  const sectors = snapshot.sectors.map((sector) => {
    const stocks = (sector.stocks || [])
      .filter((stock) => !isExcludedProduct(stock))
      .map((stock) => enrichStockFlow(stock, sector.name, now, alerts));
...
function recordAlert(stock, windowSec, tradeAmountMillion, price, alerts, now) {
  ...
  const next = {
    ...
    sector: stock.sector,
    windowSec,
    windowLabel: `${windowSec / 60}분`,
    tradeAmountMillion,
    flow60sTradeAmountMillion: stock.flow60sTradeAmountMillion,
    flow180sTradeAmountMillion: stock.flow180sTradeAmountMillion,
    volume: windowSec === 60 ? stock.flow60sVolume : stock.flow180sVolume,
    flow60sVolume: stock.flow60sVolume,
    flow180sVolume: stock.flow180sVolume,
    price,
...
async function ensureBridgeStarted() {
  ...
  if (health.httpStatus || health.login || health.provider) {
    console.log('[money-dashboard] Kiwoom bridge already reachable');
    return;
  }
  if (EXTERNAL_BRIDGE_ONLY) {
    console.warn(`[money-dashboard] external Kiwoom bridge mode enabled; not starting bundled bridge. Start Money bridge first: ${BRIDGE_URL}`);
    return;
  }

  const scriptPath = path.join(__dirname, 'bridge', 'kiwoom_bridge_flow.py');
  if (!fs.existsSync(scriptPath)) {
    console.warn(`[money-dashboard] Kiwoom bridge script not found: ${scriptPath}`);
    return;
  }

  const runtime = resolveKiwoomPythonRuntime();
  if (!runtime) {
    console.warn('[money-dashboard] Kiwoom bridge auto-start skipped: no 32-bit Python runtime was found.');
    console.warn('[money-dashboard] Install Python Windows installer (32-bit / x86), then run npm run server again.');
    console.warn('[money-dashboard] Check detected runtimes with: py -0p');
    return;
  }
  const { python, pythonArgs, label: pythonLabel } = runtime;
  const bridgeEnv = {
    ...process.env,
    ...
    ALLOW_NAVER_SECTOR: process.env.ALLOW_NAVER_SECTOR || '1',
    SECTOR_WEB_FALLBACK: process.env.SECTOR_WEB_FALLBACK || '1',
    SECTOR_WEB_TIMEOUT_SEC: process.env.SECTOR_WEB_TIMEOUT_SEC || '2.0',
  ...
  child.unref();
  console.log(`[money-dashboard] Kiwoom bridge auto-start requested with ${pythonLabel}`);
}

function resolveKiwoomPythonRuntime() {
  if (process.env.KIWOOM_PYTHON) {
    return {
      python: process.env.KIWOOM_PYTHON,
      pythonArgs: ['kiwoom_bridge_flow.py'],
      label: `${process.env.KIWOOM_PYTHON} kiwoom_bridge_flow.py`,
    };
  }

  for (const selector of ['-3.10-32', '-3.13-32', '-3.12-32', '-3.11-32', '-3-32']) {
    if (is32BitPythonSelector(selector)) {
      return {
        python: 'py',
        pythonArgs: [selector, 'kiwoom_bridge_flow.py'],
        label: `py ${selector} kiwoom_bridge_flow.py`,
      };
    }
  }
  return null;
...
function is32BitPythonSelector(selector) {
  const result = spawnSync('py', [selector, '-c', "import struct; raise SystemExit(0 if struct.calcsize('P') == 4 else 1)"], {
    cwd: __dirname,
    encoding: 'utf8',
    windowsHide: true,
  });
...
function isExcludedProduct(row) {
  if (!row || row.excluded === true) return true;
  const text = [
    row.name,
    row.code,
    row.sector,
    ...
    row.rawInfo,
    row.rawState,
    row.rawTr && JSON.stringify(row.rawTr),
    row.rawDailyDetail && JSON.stringify(row.rawDailyDetail),
  ].filter(Boolean).join(' ');
  return EXCLUDED_PRODUCT_RE.test(text);
}
...
function flattenSnapshotStocks(snapshot) {
  ...
  sectors.forEach((sector, sectorIndex) => {
    (sector.stocks || []).forEach((stock) => {
      if (isExcludedProduct(stock)) return;
      rows.push({
        ...stock,
...
function normalizeDailyAmountRankPayload(payload) {
  const rows = Array.isArray(payload.items) ? payload.items.filter((row) => !isExcludedProduct(row)) : [];
...
function isRisingRankRow(row) {
  const raw = row?.rawTr || row?.rawDailyDetail || {};
  const rawRateText = String(raw['등락률'] ?? raw['등락율'] ?? raw['전일대비율'] ?? '').trim();
  if (rawRateText && rawRateText !== '-' && rawRateText !== '--') {
    const rawRate = Number.parseFloat(rawRateText.replace(/[, %+]/g, ''));
    return Number.isFinite(rawRate) && rawRate > 0;
  }
  const rate = Number(row?.changeRate || 0);
  if (Number.isFinite(rate) && rate > 0) return true;
  const signText = `${raw['전일대비기호'] || ''}${raw['대비기호'] || ''}${raw['등락기호'] || ''}`;
  if (signText.includes('상승') || signText.includes('▲') || signText.includes('+') || signText.includes('2')) return true;
  const diff = Number.parseFloat(String(raw['전일대비'] || raw['대비'] || '').replace(/[, +]/g, ''));
