...
## SSE Opportunity Score

SSE Opportunity Score는 안전 필터가 아니라 좋은 자리 후보를 빠르게 우선순위화하는 소프트 스코어다.

안전 필터는 기존과 동일하게 유지한다.
...
위 조건은 계속 신규매수 긍정 판정을 차단한다.

Opportunity Score는 아래 항목을 점수화한다.

- 압력값 위치: 0.3~0.8 구간을 초기 회복 우수 구간으로 가산
- 기준선/진입가 회복: SSE_BASE 또는 SSE_ENTRY 회복 여부
- 눌림 후 회복: 최근 저가가 SSE_BASE/SSE_LOWER 근처까지 눌린 뒤 회복했는지
- 변동성 수축 후 회복: SSE_VOLATILITY 백분위 또는 60일 평균 대비 수축 여부
- 거래량/거래대금: 20일 평균 대비 유입 강도
- 손익비: RR1 1.2 이상, RR2가 RR1보다 큰지
...
C급 관찰: 50점 이상
대기: 50점 미만
제외: 데이터 불량, 과열, 약세 이탈 등 안전 필터상 부적합
```

셋업 코드는 다음과 같다.

```text
PULLBACK_RECLAIM: 눌림 후 기준선/진입가 회복형
BASE_RECLAIM: 기준선/진입가 회복형
COMPRESSION_BREAKOUT: 변동성 수축 후 회복형
WATCH_ENTRY: 진입가 근처 감시형
OVERHEATED_HOLD_ONLY: 과열권, 보유자 익절 관리 우선
WEAK_BREAKDOWN: 약세 이탈
DATA_BLOCKED: 데이터 검증 실패
NO_SETUP: 뚜렷한 우수 셋업 없음
```

중요 원칙:

```text
Opportunity Score가 높아도 최종 판정의 안전 필터를 우회하지 않는다.
기회 등급은 관심종목 우선순위화 용도이고, 최종 매수 긍정 판정은 기존 SSE 안전 조건을 모두 만족해야 한다.
```

Opportunity Score는 매수 신호가 아니다.

Opportunity Score는 관심종목을 어떤 순서로 다시 볼지 정하는 우선순위화 지표다. 점수가 높아도 신규매수 긍정 판정으로 자동 전환하지 않는다.

최종 신규매수 긍정 판정은 반드시 SSE Safety Filter를 통과해야 한다.

SSE Safety Filter 차단 조건은 아래와 같다.
...
아래 상황에서는 Opportunity Score가 높아도 `사라` 또는 `조건부로 사라`를 출력하지 않는다.

- `RR1 < 1.2`
- `current_price >= SSE_NO_CHASE`
- `SSE_PRESSURE >= 1.5`
- `SSE_PRESSURE < -1.0`
